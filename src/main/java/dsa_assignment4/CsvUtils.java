...
public class CsvUtils
{
		...
		throws Exception
	{
		...
		try ( Scanner file1 = new Scanner(file1Path); Scanner file2 = new Scanner(file2Path) ;
			...
			boolean flgRead1=false, flgRead2=false;
															
			row1 = formatter1.readRow(file1); 
			row2 = formatter2.readRow(file2);
			while(true){
				if(flgRead1 && row1!=null) row1 = formatter1.readRow(file1); 
				if(flgRead2 && row2!=null) row2 = formatter2.readRow(file2);
				
				if( row1!=null && row2!=null ) {
					if(comparator.compare(row1,row2)>0)  {
						formatter2.writeRow(to, row2);
						flgRead1 = false; flgRead2 = true;
					}
					else {
						formatter1.writeRow(to, row1);
						flgRead2 = false; flgRead1 = true;
					}
				}
				else if(row1!=null && row2==null) {
					formatter1.writeRow(to, row1);
					flgRead1 = true; flgRead2 = false;					
				}
				else if(row1==null && row2!=null) {
					formatter2.writeRow(to, row2);
					flgRead1 = false;  flgRead2 = true;
		...
		throws Exception
	{
		...
		Path file1Path, file2Path;
		Path temp = dataDir.resolve("temp.csv");

		// WRITE YOUR CODE HERE AND REPLACE THE RETURN STATEMENT
		if((file1Path = paths.pollFirst())==null) return false;
		if(!copyCsv(file1Path, temp)) return false;
		
		while((file2Path = paths.pollFirst())!=null) {
			if(mergePairCsv(temp, file2Path, columnName, outputPath)==false) return false;		
			if(!copyCsv(outputPath,temp)) return false;
		}
